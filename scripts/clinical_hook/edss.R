...
edss_val <- edss %>% filter(subject %in% val_pts)

edss_val$CC_1yr <- NA
edss_val$CC_2yr <- NA
for (i in 1:nrow(edss_val)) {
  p_id <- edss_val$subject[i]
  curr_date <- edss_val$visit_date[i]
  attack_subset <- MS_attack_val %>% filter(PatientID == p_id)
  edss_val$CC_1yr[i] <- ifelse(nrow(attack_subset %>% filter(onset >= curr_date & onset <= curr_date + years(1))) > 0, 1, 0)
  edss_val$CC_2yr[i] <- ifelse(nrow(attack_subset %>% filter(onset >= curr_date & onset <= curr_date + years(2))) > 0, 1, 0)
}

logreg_val_1yr <- glm(CC_1yr ~ edss, edss_val, family = binomial)
logreg_val_2yr <- glm(CC_2yr ~ edss, edss_val, family = binomial)
summary(logreg_val_1yr); summary(logreg_val_2yr)

# Correlation: all data with relapse info ---------------------------------
edss <- edss %>% filter(subject %in% unique(MS_attack$PatientID))

edss$CC_1yr <- NA
edss$CC_2yr <- NA
for (i in 1:nrow(edss)) {
  p_id <- edss$subject[i]
  curr_date <- edss$visit_date[i]
  attack_subset <- MS_attack_subs %>% filter(PatientID == p_id)
  edss$CC_1yr[i] <- ifelse(nrow(attack_subset %>% filter(onset >= curr_date & onset <= curr_date + years(1))) > 0, 1, 0)
  edss$CC_2yr[i] <- ifelse(nrow(attack_subset %>% filter(onset >= curr_date & onset <= curr_date + years(2))) > 0, 1, 0)
}

logreg_all_1yr <- glm(CC_1yr ~ edss, edss, family = binomial)
logreg_all_2yr <- glm(CC_2yr ~ edss, edss, family = binomial)
...
for (i in 1:length(tw_list)) {
  ...
  edss$CC_pred <- NA
  # Add WalkTime 
  for (j in 1:nrow(edss)) {
    p_id <- edss$subject[j]
    curr_date <- edss$visit_date[j]
    ...
    if (nrow(preds_subset) > 0) {
      edss$CC_pred[j] <- mean(preds_subset$RelapsePred)
    }
  }
  # Fit GEE 
  tmp_data <- edss[,c('subject','edss', 'CC_pred')] %>% filter(!is.na(CC_pred))
  print(paste0("N = ", nrow(tmp_data)))
  tmp_data$subject <- factor(tmp_data$subject)
  gee.fit <- gee(formula = CC_pred ~ edss, 
                 id = subject, 
                 data = tmp_data, 
                 na.action = na.omit, maxiter = 100,
                 family = quasi)
  print(tmp <- summary(gee.fit)$coefficients['edss', c('Estimate', 'Robust z')])
