...
```{r CohortTreatAttack, echo=FALSE, message=FALSE, warning=FALSE}
...
if (readin) {
  ...
  MS_cohort$LAST_VISIT_DATE = as.Date(MS_cohort$LAST_VISIT_DATE,
                                      format = "%Y-%m-%d", origin = "1900-01-01")
  MS_cohort$FIRST_CODE = as.Date(MS_cohort$FIRST_CODE, 
                                 format = "%Y-%m-%d", origin = "1900-01-01")
  MS_cohort$FIRST_DATE = as.Date(MS_cohort$FIRST_DATE, 
                                 format = "%Y-%m-%d", origin = "1900-01-01")
...
```{r ICD_CPT_CUI, echo=FALSE, message=FALSE, warning=FALSE}
# Read in/combine ICD (PheCode)/CPT (Grouped)/ CUI Codes
...
if (readin) {
  ...
  ICDPheCode   = read.csv(paste0(wkpath,"EHR/MS_AllEncounter_ICDCodes_08072019.csv"),
                          stringsAsFactors = FALSE)
  tmp = ICDPheCode$concept_cd == "LPA268"
  ICDPheCode$phecode[tmp] = "335_"
  ...
  #                        stringsAsFactors = FALSE)
  CPTGrouped  = read.csv(paste0(wkpath,"EHR/MS_AllEncounters_CPTCodes_08072019.csv"),
                        stringsAsFactors = FALSE)
...
```{r CountModelGraph, echo=FALSE, message=FALSE, warning=FALSE}
#### Encounter with unique date: generate 'intermediate_data' counts
...
# Store predicted probabilities of each model in list
glmnet.probs.list <- list()
readin = FALSE
for (i in 1:length(tw_list)) {
  tw = tw_list[i]; tp = tp_list[i]
  # tw = 24; tp = 3
  if (readin) {
    CC_comb  = read.csv(paste0("intermediate_data/CC_ICD_CPT_CUI_Count_",
                               tw,"mons_",tp,"mons.csv"),
                        stringsAsFactors = FALSE)
  ...
  # Save data for 24mons/3mons model (final model)
  if (tw == 24 & tp == 3) {
    saveRDS(CC_comb, 'intermediate_data/CC_comb_24_3.rds') # Training data
    saveRDS(CC_comb_val, 'intermediate_data/CC_comb_val_24_3.rds') # Validation data
  }
  ...
  if (readin) {
    ...
    save(glmnet.fit,
         file = paste0(wkpath3,"glmnet_cv_deviance_",tw,"mons_",tp,"mons_",rarethresh,"rarethresh.rda"))
  }
  
  # Predict and store 
  glmnet.probs.list[[paste0(tw, "_", tp)]] <- 
    predict(glmnet.fit, 
...
```{r ROCplot, echo=FALSE, message=FALSE, warning=FALSE}
# ROC: LASSO model with optimal (i.e., minimize deviance) lambda (tuning parameter) + one SE
library(purrr)
roc.list <- map(glmnet.probs.list, function(preds) {
  roc(unlist(CC_comb_val[,4]), as.numeric(preds))
  })

# Save as PDF
# pdf('plots/rocplots.pdf')
