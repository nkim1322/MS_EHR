...
.ordinal=function(ind=NULL,data, wgti=NULL, Wi=NULL, yy0 = 0.5, nopen.ind=NULL,nfold=5,K,
...
ROC.FUN.RF.revise.mycv  = function(ind=NULL,data, wgti=NULL, Wi=NULL, yy0 = 0.5, nopen.ind=NULL,K=2, FPR0=seq(.01,.99,by=.01),rtn="ALL",yes.CV=F,rep = 10, yes.seed =FALSE)
{
  nn <- nrow(data); if(is.null(wgti)){wgti=rep(1,nn)}; 
  if(!is.null(ind)){data <- data[ind,]; wgti = wgti[ind]}    
  n.set <- 1; pp = ncol(data); yyi.vmat = matrix(NA, nrow=nn, ncol=rep)
  ...
  model.full = randomForest(as.factor(data$Label) ~., data = data[,-1])
  yyi = predict(model.full, data,type = "prob")[,2]
  rochat = ROC.Est.FUN(data$Label, yyi, yy0, FPR0, wgti = wgti)
  
  if(yes.seed) {set.seed(315)}
  
  if(yes.CV) {
    nv = floor(nn/K); roc.cv = NULL; roc.cv.PD =NULL
    for(i in 1:rep){
      tmpind=sample(1:nn); data = data[tmpind,]; wgti = wgti[tmpind]
      for(k in 1:K){
        ind.v = 1:nv + (k-1)*nv; ind.t = setdiff(1:nn,ind.v)
        ...
        model.t = randomForest(as.factor(dat.t$Label) ~., data = dat.t[,-1])
        #yyi.v = as.numeric(predict(model.t, dat.v))
        yyi.v = predict(model.t, dat.v, type = "prob")[,2]
        yyi.vmat[ind.v, i] = yyi.v
        yyi.t = predict(model.t, dat.t, type = "prob")[,2]
        #auc.k =  ROC.Est.FUN(dat.v[,1],yyi.v,yy0)[1]
        roc.k = ROC.Est.FUN(dat.v$Label,yyi.v,yy0, FPR0,wgti[ind.v])
        #roc.k.PD = ROC.Est.FUN()
        #auc.cv = c(auc.cv, auc.k)
        roc.cv = cbind(roc.cv, roc.k)
      }
    }
    print(ncol(roc.cv)); #auc.cv = mean(auc.cv)
    auc.cv = roc.cv[1, ];roc.cv = apply(roc.cv,1,mean,na.rm=T)
  }
  
  rochat.auc = rochat[1]
  rochat.values = matrix(rochat[-1],ncol=6)
  
  colnames(rochat.values) = c("cutoff","pos.rate","FPR","TPR","PPV","NPV")
  roc.cv.auc =roc.cv[1]
  roc.cv.values = matrix(roc.cv[-1],ncol=6)
  colnames(roc.cv.values) = c("cutoff","pos.rate","FPR","TPR","PPV","NPV")
  return(list(ROC.hat.auc=rochat.auc, ROC.hat.values=rochat.values, ROC.cv.auc=roc.cv.auc,ROC.cv.values=roc.cv.values, Y.hat=yyi))
  
}

ROC.FUN.RF.revise.mycv.ADD  <- function(ind=NULL,data, yy0 = 0.5, nopen.ind=NULL,K=2,
                                        FPR0=seq(.01,.99,by=.01),rtn="ALL",yes.CV=F,yes.seed=F,rep=10,train_label = "Label_D", test_label = "Label_DP")
{
  ## ========================================================================= ##
  ## ==== ind: for bootstrap methods; data[ind,] returns bootstrap samples === ##
  ## ==== 1st column is binary disease status;                            ==== ##
  ...
  ## ========================================================================= ##
  nn <- nrow(data)
  if(!is.null(ind)){data <- data[ind,]}    
  n.set <- 1; pp = ncol(data)-1; yyi.vmat = matrix(NA, nrow=nn, ncol=rep)
  ## ========================= ##
  ## === Apparent Accuracy === ##
  ## ========================= ##
  data.train_label = data[, setdiff(colnames(data), test_label)]
  colnames(data.train_label)[1] = "Label"
  model.full = randomForest(as.factor(data$Label) ~., data = data.train_label[,-1])
  yyi = predict(model.full, data,type = "prob")[,2]
  rochat = ROC.Est.FUN(data[, train_label],yyi,yy0,FPR0)
  rochat.add =ROC.Est.FUN(data[, test_label],yyi,yy0,FPR0)
  if(yes.seed){set.seed(315)}
  if(yes.CV)
  {
    ## =============================== ##
    ## === K-fold cross validation === ##
    ## =============================== ##
    nv = floor(nn/K); roc.cv = NULL; roc.cv.add = NULL
    for(i in 1:rep){
      tmpind=sample(1:nn); data = data[tmpind,]; 
      for(k in 1:K){
        ind.v = 1:nv + (k-1)*nv; ind.t = setdiff(1:nn,ind.v)
        dat.t = data[ind.t,]; dat.v = data[ind.v,]            
        ## ============================================================================== ##
        ...
        ## ====             (2) Combined Marker Value (yyi.new) with Validation Data ==== ##
        ## ============================================================================== ##
        data.t.train_label = dat.t[, setdiff(colnames(dat.t), test_label)]
        data.v.train_label = dat.v[, setdiff(colnames(dat.v), test_label)]
        colnames(data.t.train_label)[1] = "Label"
        model.t = randomForest(as.factor(Label) ~., data = data.t.train_label)
        yyi.v = predict(model.t, data.v.train_label, type = "prob")[,2]; yyi.vmat[ind.v,i]=yyi.v
        yyi.t = predict(model.t, data.t.train_label, type = "prob")[,2]
        roc.k = try(ROC.Est.FUN(dat.v[,train_label],yyi.v,yy0,FPR0),silent=T)
        roc.k.add = try(ROC.Est.FUN(dat.v[,test_label],yyi.v,yy0,FPR0),silent=T)
        if(length(roc.k)>1){roc.cv = cbind(roc.cv, roc.k); roc.cv.add = cbind(roc.cv.add, roc.k.add)}
        
      }
    }
    print(ncol(roc.cv))
    auc.cv = roc.cv[1,]; auc.cv.add = roc.cv.add[1,]
    roc.cv = apply(roc.cv,1,mean,na.rm=T); roc.cv.add = apply(roc.cv.add, 1, mean, na.rm = T) 
  }else{
    roc.cv = NULL; roc.cv.add = NULL
  }
  
  ##Apparent
  rochat.auc = rochat[1]
  rochat.auc.add = rochat.add[1]
  rochat.values = matrix(rochat[-1],ncol=6)
  rochat.values.add = matrix(rochat.add[-1],ncol=6)
  ##CV
  roc.cv.auc = roc.cv[1]
  roc.cv.values = matrix(roc.cv[-1],ncol=6)
  roc.cv.auc.add = roc.cv.add[1]
  roc.cv.values.add = matrix(roc.cv.add[-1],ncol=6)
  
  colnames(rochat.values) = colnames(rochat.values.add) = colnames(roc.cv.values)=
    colnames(roc.cv.values.add) = c("cutoff","pos.rate","FPR","TPR","PPV","NPV")
  
  return(list(ROC.hat.auc=rochat.auc, ROC.hat.auc.add = rochat.auc.add,
              ROC.hat.values=rochat.values, ROC.hat.values.add = rochat.values,
              ROC.cv.auc=roc.cv.auc,ROC.cv.auc.add = roc.cv.auc.add,
              ROC.cv.values=roc.cv.values, ROC.cv.values.add = roc.cv.values.add, Y.hat=yyi))
  
}


## ADD a second label for testing
ROC.FUN.ALASSO.revise.mycv.ADD  <- function(ind=NULL,data, wgti=NULL, Wi=NULL, yy0 = 0.5, nopen.ind=NULL,K=2, FPR0=seq(.01,.99,by=.01),rtn="ALL",yes.CV=F,yes.seed=F,rep=10,regularize=T,lam.ridge=0,train_label = "Label_D", test_label = "Label_DP")
{
  ## ========================================================================= ##
  ## ==== ind: for bootstrap methods; data[ind,] returns bootstrap samples === ##
  ## ==== 1st column is binary disease status;                            ==== ##
  ...
  betahat = try(Est.ALASSO.revise.GLM(data.train_label,Wi=wgti, rtn="EST",regularize=regularize,nopen.ind=nopen.ind),silent=T)
  if(length(betahat)>1)
  {
    yyi = g.logit(cbind(1,as.matrix(data.train_label[,-1]))%*%betahat[1:pp]); 
    q.nonzero = sum(abs(betahat[2:(pp)])!=0)
    if(q.nonzero==0){rochat = NULL; rochat.add = NULL}else{
      rochat = ROC.Est.FUN(data[, train_label],yyi,yy0,FPR0,wgti=wgti)
      rochat.add =ROC.Est.FUN(data[, test_label],yyi,yy0,FPR0,wgti=wgti)}
    if(yes.seed){set.seed(315)}
    if(yes.CV & q.nonzero >0)
    {
      ## =============================== ##
      ...
      for(i in 1:rep){
        ...
        for(k in 1:K){
          ...
          beta.t = try(Est.ALASSO.revise.GLM(data.t.train_label,Wi=wgti[ind.t],rtn="EST",regularize=regularize,nopen.ind=nopen.ind),silent=T)
          if(length(beta.t)>1){
            beta.t = beta.t[1:pp]
            yyi.v = g.logit(cbind(1,as.matrix(data.v.train_label[,-1]))%*%beta.t);  yyi.vmat[ind.v,i]=yyi.v
            yyi.t = g.logit(cbind(1,as.matrix(data.t.train_label[,-1]))%*%beta.t)
            roc.k = try(ROC.Est.FUN(dat.v[,train_label],yyi.v,yy0,FPR0,wgti[ind.v]),silent=T)
            roc.k.add = try(ROC.Est.FUN(dat.v[,test_label],yyi.v,yy0,FPR0,wgti[ind.v]),silent=T)
            if(length(roc.k)>1){roc.cv = cbind(roc.cv, roc.k); roc.cv.add = cbind(roc.cv.add, roc.k.add)}
          }
        }
      }
    ...
    roc.cv=beta.hat=roc.cv.add=NULL
  }
  if(rtn=="EST"){
    if(yes.CV){out=c(roc.cv,roc.cv.add,betahat)}else{out=c(rochat,rochat.add,betahat)}
    return(out)
  }else{return(list("rochat"=rochat, "rochat.add" = rochat.add,
                    "roc.cv"=roc.cv, "roc.cv.add" = roc.cv.add,
                    "beta"=betahat, "Si"=yyi, "yyi.v"=yyi.vmat,
                    "auc.cv"=auc.cv, "auc.cv.add" = auc.cv.add))}
}




ROC.SSL.FUN.ALASSO.revise.mycv  <- function(ind=NULL,data, wgti=NULL, Wi=NULL, yy0 = 0.5, nopen.ind=NULL,K=2,
                                        FPR0=seq(.01,.99,by=.01),rtn="ALL",yes.CV=F,yes.seed=F,rep=10,regularize=T,lam.ridge=0)
{
  ## ========================================================================= ##
  ## ==== ind: for bootstrap methods; data[ind,] returns bootstrap samples === ##
  ...
  if(length(betahat)>1)
  {
    yyi = g.logit(cbind(1,as.matrix(data[,-1]))%*%betahat[1:pp]); 
    q.nonzero = sum(abs(betahat[2:(pp)])!=0)
    if(q.nonzero==0){rochat = NULL}else{
      tmp.dat = cbind.data.frame("Y" = data[,1],"S" = yyi); tmp.rochat = roc.ssl.FUN(tmp.dat, method = "EM"); ROC.hat.auc = tmp.rochat$auc; ROC.hat.values=  tmp.rochat$roc}
    if(yes.seed){set.seed(315)}
    if(yes.CV & q.nonzero >0)
    {
      ## =============================== ##
      ## === K-fold cross validation === ##
      ## =============================== ##
      nv = floor(nn/K); roc.cv = NULL; auc.cv = NULL
      for(i in 1:rep){
        tmpind=sample(1:nn); data = data[tmpind,]; wgti = wgti[tmpind]
        for(k in 1:K){
          ind.v = 1:nv + (k-1)*nv; ind.t = setdiff(1:nn,ind.v)
          ...
          if(length(beta.t)>1){
            ...
            yyi.v = g.logit(cbind(1,as.matrix(dat.v[,-1]))%*%beta.t);  yyi.vmat[ind.v,i]=yyi.v
            yyi.t = g.logit(cbind(1,as.matrix(dat.t[,-1]))%*%beta.t)
            tmp.dat = cbind.data.frame("Y" = dat.v[,1], "S" = yyi.v)
            rochat.tmp.k = roc.ssl.FUN(tmp.dat, method = "EM")
            auc.k = rochat.tmp.k$auc
            roc.k = as.vector(t(rochat.tmp.k$roc))
            auc.cv = c(auc.cv, auc.k)
            roc.cv = cbind(roc.cv, roc.k)
          }
        }
      }
      print(ncol(roc.cv)); roc.cv = apply(roc.cv,1,mean,na.rm=T)  
    }else{
      roc.cv = NULL
    }
  }else{
  ...
  ROC.cv.auc = mean(auc.cv, na.rm = TRUE)
  ROC.cv.values = matrix(roc.cv,ncol=6)
  colnames(ROC.cv.values) = c("cutoff","pos.rate","FPR","TPR","PPV","NPV")
  ...
  if(rtn=="EST"){
    ...
    return(list("ROC.hat.auc"=ROC.hat.auc, "ROC.hat.values" = ROC.hat.values,
                "beta"=betahat, "Si"=yyi, 
                "auc.cv"=auc.cv, "yyi.v"=yyi.vmat,
                "ROC.cv.auc" = ROC.cv.auc,
                "ROC.cv.values" = ROC.cv.values))}
}

ROC.FUN.ALASSO.revise.mycv.plain  <- function(ind=NULL,data, wgti=NULL, Wi=NULL, yy0 = 0.5, nopen.ind=NULL,K=2,
                                              FPR0=seq(.01,.99,by=.01),rtn="ALL",yes.CV=F,yes.seed=F,regularize=T,lam.ridge=0)
{
  ## ========================================================================= ##
  ## ==== ind: for bootstrap methods; data[ind,] returns bootstrap samples === ##
  ## ==== 1st column is binary disease status;                            ==== ##
  ...
  if(length(betahat)>1)
  {
    yyi = g.logit(cbind(1,as.matrix(data[,-1]))%*%betahat[1:pp]); 
    q.nonzero = sum(abs(betahat[2:(pp)])!=0)
    if(q.nonzero==0){rochat = NULL}else{rochat = ROC.Est.FUN(data[,1],yyi,yy0=sort(unique(yyi)),wgti=wgti)}
    if(yes.seed){set.seed(315)}
    if(yes.CV & q.nonzero >0)
    {
      ## =============================== ##
      ## === K-fold cross validation === ##
      ## =============================== ##
      nv = floor(nn/K); pred.all = rep(NA, length = nn)
      for(k in 1:K) {
        ind.v = 1:nv + (k-1)*nv; ind.t = setdiff(1:nn,ind.v)
        dat.t = data[ind.t,]; dat.v = data[ind.v,]
        beta.t = try(Est.ALASSO.revise.GLM(dat.t,Wi=wgti[ind.t],rtn="EST",regularize=regularize,nopen.ind=nopen.ind),silent=T)
        if(length(beta.t)>1){
          ...
          pred.all[ind.v] = yyi.v
        }      
      }
      roc.cv = try(ROC.Est.FUN(data[,1],pred.all,yy0 = sort(unique(pred.all)) ,wgti = wgti),silent=T)
      auc.cv = roc.cv[1]
    }
  } 
  if(rtn == "EST") {
    ...
    return(list("rochat"=rochat, "roc.cv"=roc.cv, "beta"=betahat, "Si"=yyi, "auc.cv"=auc.cv, "yyi.v"=pred.all))
  }
}


ROC.FUN.ALASSO.revise.mycv.ADD.plain = function(ind=NULL,data, wgti=NULL, Wi=NULL, yy0 = 0.5, nopen.ind=NULL,K=2, FPR0=seq(.01,.99,by=.01),rtn="ALL",yes.CV=F,yes.seed=F,regularize=T,lam.ridge=0,train_label = "Label_D", test_label = "Label_DP")
{
  ## ========================================================================= ##
  ## ==== ind: for bootstrap methods; data[ind,] returns bootstrap samples === ##
  ## ==== 1st column is binary disease status;                            ==== ##
  ...
  if(length(betahat)>1)
  {
    ...
    if(yes.CV & q.nonzero >0)
    {
      ...
      ## === K-fold cross validation === ##
      ## =============================== ##
      nv = floor(nn/K); roc.cv = NULL; roc.cv.add = NULL; pred.all= rep(NA, nrow = nn)
      for(k in 1:K) {
        ind.v = 1:nv + (k-1)*nv; ind.t = setdiff(1:nn,ind.v)
        dat.t = data[ind.t,]; dat.v = data[ind.v,]            
        ## ============================================================================== ##
        ...
        if(length(beta.t)>1){
          beta.t = beta.t[1:pp]
          yyi.v = g.logit(cbind(1,as.matrix(data.v.train_label[,-1]))%*%beta.t); 
          pred.all[ind.v]=yyi.v
        }
      }
      roc.cv = try(ROC.Est.FUN(data[,train_label],pred.all,yy0 = sort(unique(pred.all))),silent=T)
      roc.cv.add = try(ROC.Est.FUN(data[, test_label], pred.all, yy0 = sort(unique(pred.all))), silent = T)
    }
    auc.cv = roc.cv[1]; auc.cv.add = roc.cv.add[1]
  }
  ...
  if(rtn=="EST"){
    ...
    return(out)
                    ...
                    "roc.cv"=roc.cv, "roc.cv.add" = roc.cv.add,
                    "beta"=betahat, "Si"=yyi, "yyi.v"=pred.all,
                    "auc.cv"=auc.cv, "auc.cv.add" = auc.cv.add))}
}
