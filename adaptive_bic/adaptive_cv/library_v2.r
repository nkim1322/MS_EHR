...
ROC.Est.FUN <- function(Di,yyi,yy0,fpr0=NULL,wgti=NULL,yes.smooth=F, na.rm = FALSE)
  { 
  ## make sure Di and yyi matched before using this funciton
  if(na.rm) {
    na.rows = which(is.na(Di))
    Di = Di[-na.rows]
    yyi = yyi[-na.rows]
  }
...
convert <- function(fit, cv.value = FALSE) {
	...
	if(cv.value == FALSE) {
	  ...
	  return(list(ROC.hat.auc=rochat.auc, ROC.hat.values=rochat.values, ROC.cv.auc=roc.cv.auc,ROC.cv.values=roc.cv.values, beta=betahat, Y.hat=fit$Si, Y.cv = fit$yyi.v))
	}
}

convert.add <- function(fit, cv.value = FALSE) {
  ##Apparent
  rochat.auc = fit$rochat[1]
  rochat.auc.add = fit$rochat.add[1]
  rochat.values = matrix(fit$rochat[-1],ncol=6)
  rochat.values.add = matrix(fit$rochat.add[-1],ncol=6)
  ##CV
  roc.cv.auc = fit$roc.cv[1]
  roc.cv.values = matrix(fit$roc.cv[-1],ncol=6)
  roc.cv.auc.add = fit$roc.cv.add[1]
  roc.cv.values.add = matrix(fit$roc.cv.add[-1],ncol=6)
  ...
  if(cv.value == FALSE) {
    return(list(ROC.hat.auc=rochat.auc, ROC.hat.auc.add = rochat.auc.add,
                ...
                ROC.cv.values=roc.cv.values, ROC.cv.values.add = roc.cv.values.add,
                beta=betahat, Y.hat=fit$Si))
  }
  else {
    return(list(ROC.hat.auc=rochat.auc, ROC.hat.auc.add = rochat.auc.add,
                ROC.hat.values=rochat.values, ROC.hat.values.add = rochat.values,
                ROC.cv.auc=roc.cv.auc,ROC.cv.auc.add = roc.cv.auc.add,
                ROC.cv.values=roc.cv.values, ROC.cv.values.add = roc.cv.values.add,
                beta=betahat, Y.hat=fit$Si, Y.cv = fit$yyi.v))
  }
}
