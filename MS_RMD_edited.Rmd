...
<!--Changed normalized code to adjusted (notes, any ICD, baseline) and adjusted 2 (notes, any ICD)-->

## Abstract
__Background__: Disease-modifying treatments (DMT) reduce relapse rates in multiple sclerosis (MS).

__Objective__: To investigate the temporal trend of disease activity over time in people with MS in relation to DMT usage. 

__Design, Setting, and Participants__:  We analyzed prospectively collected research registry data from a well-characterized clinic cohort based in Boston from 2000 to 2016, and integrated with electronic health records data.

__Main Outcomes and Measures__: We assessed the temporal trend of relapse rate (any, clinical or radiographic), percentage of the cohort receiving any DMT.  We additionally assessed the trend of average 25-OH vitamin D level, average disease duration, average number of diagnostic code for MS and average number of any diagnostic code. 

__Results__: We assessed the research registry data of xx people with MS between 2000 and 2016: mean age = xx years, mean disease duration = xx years, female to male ratio = xx :1. We found...

__Conclusion and Relevance__: This detailed examination of relapse rate in relation to MS treatments confirms the benefit of DMT, and...

__Trial Registration__:  N/A

__Funding__: NINDS NS098023

## Introduction 
Multiple sclerosis (MS) is a chronic neurological disease with a high socioeconomic burden [@noteworthy2000medical;@compston2008multiple;@asche2010all;@hartung2015cost]. Since the approval of the first disease-modifying therapy (DMT) in 1993, MS treatment has shifted from primarily managing acute relapses to reducing disease activity and delaying disability as DMTs have become the standard of care for people with MS[@hartung2015cost]. Currently, there are at least 15 Food and Drug Administration (FDA) approved DMTs, including injectable, oral and infusion options[@hartung2015cost].

...

In this study, we report the trends of disease activity among people with MS in a prospective clinic cohort from 2000-2016...

## Methods

### Study Population
In this study, we obtained research registry data from the Comprehensive Longitudinal Investigation of Multiple Sclerosis at the Brigham and Women's Hospital (CLIMB) study based at Boston MA, from 2000-2016. We additionally obtained electronic health records data for patients in the CLIMB study from the Partners HealthCare[@xia2013modeling].  The Institutional Review Board of Partners HealthCare approved the use of research registry data and electronic health records data. 

### Analysis of incidence relapse rate, DMT usage and other trends
...
Information from the EHR data are extracted and mapped to the CLIMB data with the help of the i2b2 mapping table to improve the accuracy of the CLIMB data. Specifically, the first and last ICD code dates from the EHR data in combination with the enrollment/first diagnosis date and last visit date from the CLIMB data lead to a more precise estimation on follow-up period. The resulting start/end of follow-up period are further manipulated using the standard rules, i.e., entries with missing years are deleted while the entries with missing months and days are imputed by July and 15th, respectively. Besides, vitamin data are extracted and analyzed to explore its potential relationship with MS.

The patients characteristics, such as total number of patients, percentage of females, and median and IQR duration of follow-up, are calcuted among CLIMB data and EHR data, separately. Missing values in sex and race are excluded in the calculation as the missing rates are 0.4% and 2.9%, respectively. 

The incident, clinical, and radiographic relapse rates over 2000-2016 and the corresponding 95% normal CIs were computed. The former one is given in Figure 1 while the later two are provided in the supplimentary material. The 95% CI for the overall anualized relapse rate during 2000-2016 are calculated using the bootstrap method and provided in Table 1.

The trend of any DMT usage and each of the DMT usage, along with the normal CIs, are summarized in Figure 1 and 2, respectively. Table 2 listed the mean treatment duration and corresponding normal CIs for each of the DMT of interest.

The trend of estimated incident relapse rate relative to the placebo is given in Figure 1. There are two factors influence the relative relapse rate. The effectiveness of and the proportion of patients under each DMT. Specifically, 
patients under a more effective drug will have a lower chance of replase compared to that under a less effective drug. Besides, the higher proportion of patients under an effective drug in the cohort it is, the fewer relapses there will be. Let $r_i$ be the reduction rate of the $i$th DMT and $p_i$ be the proportion of the patients under the $i$th DMT. The overall incident relapse rate is estimated by 
$$\sum_i (1-r_i)p_i.$$
Here, the $r_i$ is obtained by extracting the reduction rates of all the DMTs from various literatures and then taking the average. See Table 3 for the specific values of $r_i$ used in the calculation.

The trend of median vitamin D level and median disease duration are provided in Figure 1, along with the normal CIs, due to the heavy skewness in the data.

The average number of MS diagnostic code and any diagnostic code are calculated as comparisons to the trend of the incident relapse rate, and are shown in Figure 3.

All the data analysis are performed using R version 3.4.2 (? do we need to specify the version?).


## Results
### Demographics
From 2000 to 2016, xx enrolled in the CLIMB cohort. The CLIMB cohort represents a predominant subset of a "virtual cohort" of MS derived from the electronic health records data.  The patient characteristics are shown in Table 1.  Consistent with the EHR cohort, the mean age in the CLIMB cohort was xx years old, and the female to male ratio was xx:1.  The mean disease duration in the CLIMB cohort was xx.  The mean follow-up duration was xx in the CLIMB cohort and xx in the EHR cohort.

### Trend of Incident Relapse Rate
.... 

### Trend of Disease-Modifying Treatment Prescription 
...

### Trend of 25-OH Vitamin D Level
...

### Trend of Diagnostic Code for MS
...


## Discussion
In this study , we examined the temporal trends of relapse rate in a large well-characterized prospective clinic-based cohort of MS. Our main findings are...
...
## Funding
Dr. Xia was a recipient of the Clinician Scientist Development Award from the National Multiple Sclerosis Society and the American Academy of Neurology and is supported by NIH K08-NS079493. The funding organizations and sponsors played no role in any of the following: design and conduct of the study; collection, management, analysis, and interpretation of the data; and preparation, review, or approval of the manuscript; and decision to submit the manuscript for publication.
...
}) # only extract treatment with >=100 patients
...
Trt_Used_Table2 <- b_copy$Treatment_Category
...
result        = with(MS_cohort,c(nrow(MS_cohort), # num of patients
                                 sum(unique(MS_map$i2b2[match(MS_cohort$PatientID,MS_map$PatientID)])%in%unique(MS_EHR$PatientNum)) # in EHR
))
...
sims     = 500
if (readinboot) {
  meanrate = read.table(paste0(wkpath2,"BootstrapResult/meanrate.dat"),header = TRUE)
} else{
  set.seed(1234)
  ############!!!!!!!!!!
  NP = length(PatientID)
...
result = rbind(result,c(paste0(round(mean(rate),3)," (",
...
kable(result, digits = 2, caption = "Table 1: Demographics of the Study Population")
```


<!--Supp Table. Demographics of the ever treated vs never treated.-->

```{r Demographics, echo = FALSE, message = FALSE, warning=FALSE}
result = c(nrow(EverTr), # num of patients
           nrow(NeverTr),NA) 
result = as.character(result) 
result = c(result,sum(unique(MS_map$i2b2[match(EverTr$PatientID,MS_map$PatientID)])%in%unique(MS_EHR$PatientNum)),sum(unique(MS_map$i2b2[match(NeverTr$PatientID,MS_map$PatientID)])%in%unique(MS_EHR$PatientNum)),NA) # num in EMR
...
colnames(result) = c("Ever Treated","Never Treated","p-value for heter")
...
row.names(result) = c("Total number of patients", "Num belongs to EHR",
                      ...
                      "% receiving treatments",
                      "Average Annualized relapse rate 2000-2016")
kable(result, digits=2,caption = "Supplementary Table: Demographics of the ever treated vs never treated.")
```
...
Trt_brand_Table2 = sapply(PatientID,function(x){
  tmp2 = MS_trt[MS_trt$PatientID==x,]
  if(dim(tmp2)[1]>0){
    sapply(1:length(Trt_brand_copy),function(j){
      a   = 0
      for(k in 1:length(Trt_brand_copy[[j]])){
        tmp3 = tmp2[grepl(Trt_brand_copy[[j]][k],tmp2$medication_desc),]
        a = a+sum(with(tmp3,(stop_year-start_year)*12+(stop_month-start_month)+(stop_day-start_day)/30)) 
      }
      a
    })
  } else{numeric(length(Trt_brand_copy))}
})
Trt_dura2 = data.frame(PatientID=PatientID,t(Trt_brand_Table2),stringsAsFactors=FALSE)
colnames(Trt_dura2)[-1] <- Trt_Used_Table2
...
b2    = cbind(b_copy,paste0(round(tmp,2),
                      " (",signif(tmp2,3), ", ", signif(tmp3, 3), ")"))
...
b22    = cbind(b_copy,paste0(round(tmp,2),
                      " (",round(tmp2,2), ")"))
...
num_trt     = sapply(1:length(year),function(i) apply(Combined[0:(length(PatientID)-1)*length(year)+i,Trt_Used],2,sum))
num_trt     = data.frame(year = year, t(num_trt))
rate_trt_any = sapply(1:length(year),function(i) sum(apply(Combined[0:(length(PatientID)-1)*length(year)+i,Trt_Used],1,sum)>0))/num_p  
...
tmp       = apply(Combined[,Trt_Used]==0,1,all)
...
if (readinboot) {
  ...
  boot = replicate(n=sims,expr = {
    ...
    num_p_boot    = sapply(1:length(year),function(i) with(Combined_boot[0:(length(PatientID)-1)*length(year)+i,],
                                                           sum(Enroll==1, na.rm =TRUE)))
    rate_trt_any_boot = sapply(1:length(year),function(i) sum(apply(Combined_boot[0:(length(PatientID)-1)*length(year)+i,Trt_Used],1,sum)>0,na.rm=TRUE))/num_p_boot  ###!!!!!!!! take care of NAs
    
    tmp            = apply(Combined_boot[,Trt_Used]==0,1,all)
...
colnames(trend)  = c("est","bootsd","P-value")
...
p_1A = ggplot(df, aes(year,rate))+
  ...
  annotate("text", x = 2013, y=0.22, label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend["Incident Rate",3])),family = "Times")
...
p_1B = ggplot(rate_trt_any,aes(x=year, y=rate))+
  ...
  theme(axis.text.x = element_blank(),axis.title.x=element_blank(),
        text=element_text(size=12,family="Times"))+
  annotate("text", x = 2013, y=0.4, label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend["Trt Percent",3])),family="Times")
...
p_1C = ggplot(PlaceboRR, aes(x=year, y=rate, colour=TorUT))+
  ...
  theme(axis.text.x = element_blank(),axis.title.x=element_blank(),
        text=element_text(size=12,family="Times"))+
  annotate("text", x = 2012, y=0.02, label = paste0("P-value for linear trend; Untreated = ",sprintf("%4.1e",trend["Untreated Relapse Rate",3])),family="Times")+
  annotate("text", x = 2012, y=0.22, label = paste0("P-value for linear trend; Treated = ",sprintf("%4.1e",trend["Treated Relapse Rate",3])),family="Times")
...
p_1D = ggplot(AvgDisDur, aes(x=year, y=AvgDisDur))+
  ...
  theme(axis.text.x = element_blank(),axis.title.x=element_blank(),
        text=element_text(size=12,family="Times")) +
  annotate("text", x = 2013, y=7.5, label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend["Dis Duration",3])),family="Times")
...
p_1E = ggplot(VD_CI, aes(x=year, y=VD))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text", x = 2013, y=25, label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend["VD",3])),family="Times")

tiff("paper1/Fig1.tiff", units="in", width=7, height=9, res=300)
grid.newpage()
grid.draw(ggarrange(p_1A,p_1B,p_1C,p_1D,p_1E,ncol=1))
dev.off()

### p-value table for the trend
trend = sprintf("%4.1e", trend)
trend = matrix(trend,ncol=3)
colnames(trend)  = c("est","bootsd","P-value")
...
# rate_trt <- tmp
Trt_Used_fig2 <- Trt_Used[2:6]
Trt_Used_fig2[6] <- "rituximab"
Trt_Used_fig2[7] <- Trt_Used[7]
...
p1 = ggplot(rate_trt,aes(x=year, y=rate, colour=treatment)) +
  ...
  scale_color_manual(name = "DMT", values=c("red", "deepskyblue", "green", "magenta", "goldenrod", "gray48", "orange"),
  ...
  scale_fill_manual(name = "DMT", values=c("red", "deepskyblue", "green", "magenta", "goldenrod", "gray48", "orange"),
                    breaks=Trt_Used_fig2,
                    labels=c("Dimethyl Fumarate","Fingolimod","Glatiramer Acetate", 
                             "Interferon","Natalizumab","Rituximab","Teriflunomide"))+
...
colnames(trend3)  = c("est","bootsd","P-value")
...
p_3A = ggplot(code340,aes(x=year, y=rate))+
  ...
  theme(axis.text.x.top = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text", x = 2013, y = 2.2, label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend3["code340",3])),family="Times")
...
p_3B = ggplot(anyICD,aes(x=year, y=rate))+
  ...
  theme(axis.text.x = element_blank(),axis.title.x=element_blank(),
        text=element_text(size=12,family="Times"))+
  annotate("text", x = 2013, y = 7, label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend3["anyICD",3])),family="Times")
...
p_3C = ggplot(code340_Adj,aes(x=year, y=rate))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text", x = 2013, y = 5, label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend3["code340_adj",3])),family="Times")
...
colnames(trend4)  = c("est","bootsd","P-value")
...
p_4A = ggplot(MultiSc_NLP, aes(year,avg))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text",x=2013,y=0.5,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend4["MultiSc",3])),family="Times")
...
p_4B = ggplot(Relapse_NLP, aes(year,avg,colour=Group))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text",x=2013,y=0.8,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend4["Relapse",3])),family="Times")+
  annotate("text",x=2013,y=0.3,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend4["MultiScRelapse",3])),family="Times",colour = "red")
...
p_4C = ggplot(DMT_NLP, aes(year,avg,colour=Group))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text",x=2013,y=0.9,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend4["DMT",3])),family="Times")+
  annotate("text",x=2013,y=0.5,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend4["MultiScDMT",3])),family="Times",colour = "red")
...
p_4D = ggplot(MultiSc_Adj_NLP, aes(year,avg))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text",x=2013,y=0.6,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend4["MultiSc_Adj",3])),family="Times")
...
p_4E = ggplot(Relapse_Adj_NLP, aes(year,avg,colour=Group))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text",x=2013,y=0.7,label = paste0("P-value for linear trend < 1e-323"),family="Times")+
  annotate("text",x=2013,y=0.3,label = paste0("P-value for linear trend < 1e-323"),family="Times",colour = "red")
...
p_4F = ggplot(DMT_Adj_NLP, aes(year,avg,colour=Group))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text",x=2013,y=0.9,label = paste0("P-value for linear trend < 1e-323"),family="Times")+
  annotate("text",x=2013,y=0.5,label = paste0("P-value for linear trend < 1e-323"),family="Times",colour = "red")
...
colnames(trend5)  = c("est","bootsd","P-value")
...
p_5A = ggplot(MRI_CPT,aes(x=year, y=avg,colour=Group))+
  ...
  annotate("text",x=2013,y=1,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["MRI",3])),family="Times")+
  annotate("text",x=2013,y=0.4,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["MRI_Prop",3])),family="Times",colour = "red")
...
p_5B = ggplot(HD_CPT,aes(x=year, y=avg,colour=Group))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"),
        legend.position = "none")+
  annotate("text",x=2013,y=0.25,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["HD",3])),family="Times")+
  annotate("text",x=2013,y=0,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["HD_Prop",3])),family="Times",colour = "red")
...
p_5C = ggplot(ED_CPT,aes(x=year, y=avg,colour=Group))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"),
        legend.position = "none")+
  annotate("text",x=2013,y=0.25,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["ED",3])),family="Times")+
  annotate("text",x=2013,y=0,label = paste0("P-value for linear trend = ",
                                            sprintf("%4.1e",trend5["ED",3])),family="Times",colour = "red")
...
p_5D = ggplot(Solu_CPT,aes(x=year, y=avg))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text",x=2014,y=1.25,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["Solu",3])),family="Times")
...
p_5E = ggplot(MRI_Adj_CPT,aes(x=year, y=avg, colour=Group))+
  ...
  theme(axis.text.x = element_text(size=8),
        ...
        legend.position = "none")+
  annotate("text",x=2013,y=1.0,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["MRI_Adj",3])),family="Times")+
  annotate("text",x=2013,y=0.2,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["MRI_Prop",3])),family="Times",colour="red")
# 
...
p_5F = ggplot(HD_Adj_CPT,aes(x=year, y=avg, colour=Group))+
  ...
  theme(axis.text.x = element_text(size=8),
        ...
        axis.title.y.right = element_text(color = "red"),
        legend.position = "none")+
  annotate("text",x=2013,y=0.25,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["HD_Adj",3])),family="Times")+
  annotate("text",x=2013,y=0,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["HD_Prop",3])),family="Times",colour="red")
...
p_5G = ggplot(ED_Adj_CPT,aes(x=year, y=avg, colour=Group))+
  ...
  theme(axis.text.x = element_text(size=8),
        ...
        axis.title.y.right = element_text(color = "red"),
        legend.position = "none")+
  annotate("text",x=2013,y=0.25,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["ED_Adj",3])),family="Times")+
  annotate("text",x=2013,y=0,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["ED_Prop",3])),family="Times",colour="red")
...
p_5H = ggplot(Solu_Adj_CPT,aes(x=year, y=avg))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text",x=2008,y=0.3,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trend5["Solu_Adj",3])),family="Times")
...
p_cli = ggplot(df, aes(year,rate))+
  ...
  theme(axis.text.x.top = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text",x=2003,y=0.05,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trendS[3])),family="Times")
...
bootsd  = sd(bootest)
trendS  = rbind(trendS,c(est,bootsd,2*pnorm(-abs(est/bootsd))))
...
p_rad = ggplot(df, aes(year,rate))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text",x=2003,y=0.1,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trendS[2,3])),family="Times")
...
trendS = matrix(trendS,ncol=3)
colnames(trendS)  = c("est","bootsd","P-value")
...
tmp     = cbind(tmp,qnorm(0.975)*apply(ITF_boot[year>=2006,2,],1,sd))
tmp[year<2006,] = NA
...
bootsd   = apply(est_boot,2,sd)
trendS2 = rbind(trendS2,cbind(est,bootsd,2*pnorm(-abs(est/bootsd))))
colnames(trendS2)  = c("est","bootsd","P-value")
row.names(trendS2) = c("Methylprednisolone","Methylprednisolone_Adj",
                       ITFvars,"Interferon_Adj")
...
p_S2 = ggplot(ITF_Adj,aes(x=year, y=rate))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text",x=2008,y=1.,label = paste0("P-value for linear trend = ",sprintf("%4.1e",trendS2["Interferon_Adj",3])),family="Times")
...
colnames(trendS3)  = c("est","bootsd","P-value")
...
p_S3A = ggplot(df,aes(x=year, y=freq))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text", x = 2003, y=0.35, label = paste0("P-value for linear trend = ",sprintf("%4.1e",trendS3["MultiSc.Raw",3])),family = "Times")
...
p_S3B = ggplot(df,aes(x=year, y=freq))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text", x = 2003, y=0.175, label = paste0("P-value for linear trend = ",sprintf("%4.1e",trendS3["Relapse.Raw",3])),family = "Times")
...
p_S3C = ggplot(df,aes(x=year, y=freq))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times"))+
  annotate("text", x = 2003, y=0.4, label = paste0("P-value for linear trend = ",sprintf("%4.1e",trendS3["DMT.Raw",3])),family = "Times")
...
standard_eff <- c("dimethyl_fumarate","fingolimod","glatiramer_acetate","interferon","teriflunomide")
num_trt     = sapply(1:length(year),function(i) apply(Combined[0:(length(PatientID)-1)*length(year)+i,Trt_Used],2,sum))
num_trt     = data.frame(year = year, t(num_trt))
num_trt <- cbind(num_trt$year, with(num_trt, rituximab+natalizumab), with(num_trt, dimethyl_fumarate+fingolimod+glatiramer_acetate+interferon+teriflunomide))
...
colnames(trendS4)  = c("est","bootsd","P-value")
...
p_S4A = ggplot(rate_trt_grouped, aes(x=year, y=rate, group=group))+
  ...
  annotate("text", x = 2009, y=0.2, label = paste0("P-value for linear trend; High-Efficacy = ",sprintf("%4.1e",as.numeric(trendS4["Trt Higher Percent",3]))),family="Times",size=3)+
  annotate("text", x = 2006.5, y=0.7, label = paste0("P-value for linear trend; Standard-Efficacy = ",sprintf("%4.1e",as.numeric(trendS4["Trt Standard Percent",3]))),family="Times",size=3)
...
p_S4B = ggplot(PlaceboRR, aes(x=year, y=rate, colour=TorUT))+
  ...
  theme(axis.text.x = element_text(size=8),
        text=element_text(size=12,family="Times")) +
  annotate("text", x = 2009, y=0.02, label = paste0("P-value for linear trend; Untreated = ",sprintf("%4.1e",trendS4 ["Untreated Relapse Rate",3])),family="Times",size=3)+
  annotate("text", x = 2012.5, y=0.35, label = paste0("P-value for linear trend; Treated Standard-Efficacy= ",sprintf("%4.1e",trendS4["Treated Standard Relapse Rate",3])),family="Times",size=3)+
  annotate("text", x = 2009, y=0.5, label = paste0("P-value for linear trend; Treated High-Efficacy= ",sprintf("%4.1e",trendS4["Treated Higher Relapse Rate",3])),family="Times",size=3)
